...
public class MyAdapter_left extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public MyAdapter_left(Context context, List<CatagoryBean.DataBean> list) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        //设置种类图片
        if (list.get(position).getIcon().length()>0) {
          //  Picasso.with(context).load(MyUtils.Unicode2GBK(list.get(position).getImage())).into(((MyLeftViewHolder) holder).iv_type_pic);
            Glide.with(context).load(list.get(position).getIcon()).into(((MyLeftViewHolder) holder).iv_type_pic);
        }
    }
